...
- the currently playing track is marked with a right-aligned `◆` at the end of its title line in both results-list (SEARCH) and queue-list (NORMAL); the title is padded so `◆` lands at `widthCols - 1`; the marker must be absent when `playback.nowPlaying` is null; queue-list additionally requires the track at `playingIndex` to match `playback.nowPlaying.id` (`isQueueTrackPlaying`) so a stale `playingIndex` never shows a marker
...
- HelpModal uses `Modal` base with `strategy: "centered"` and content-fit sizing (width min 48 / max 82, height = content + chrome, both capped to the screen) so the full keymap is visible without clipping on a 24-row terminal; ThemePicker uses `Modal` base with `strategy: "centered"`, `width=60`, `heightFraction=0.6`
- the help modal documents all six modes and the complete keymap/command set (mode keys, queue keybinds, Ctrl+P/A/S, library/logs/settings keys, every registered `:command`); a feature shipped without a help entry is a discoverability regression
...
- SETTINGS mode: `h`/`l` move between category tabs (`moveSettingsCategoryPrev` / `moveSettingsCategoryNext`); `j`/`k` move between items in the active category (`moveSettingsItemDown` / `moveSettingsItemUp`); `←`/`→` trigger `onChange` with direction `-1`/`+1` on the selected item; changes dispatch immediately and persist via `saveConfigThunk` on every value change; `Esc` exits SETTINGS back to NORMAL; command input (`:`) remains available — the `commandActive` block must run before the settings key handler so settings navigation never swallows command-buffer characters
...
- every statusline message auto-clears after `settings.statusTimeoutMs` via the status-timeout listener middleware (`src/state/status-timeout/status-timeout.ts`); a newer message restarts the countdown; component-level status timers are a regression
- `Ctrl+C` quits through the full cleanup path (`requestQuit` → `destroy`: stop mpv/cava, unload audio routes, persist config/library); opentui `exitOnCtrlC` stays disabled and `SIGINT`/`SIGTERM` route to the same `destroy`
