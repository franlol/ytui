...
Executable command definitions are stored in `.opencode/commands/*.md` and `.claude/commands/*.md`.
This folder documents command behavior and governance requirements.

All commands:
- start with `/`
- produce output following `../contracts/output-format.md`
- execute through an assigned runtime command definition in `.opencode/commands/*.md` or `.claude/commands/*.md`
