...
- SETTINGS mode: two-pane layout (categories left, items right); `j`/`k` navigate within focused pane; `l`/`→`/Enter moves focus to items; `h`/Esc moves focus back to categories; `←`/`→` changes the focused setting value (cycle enum / toggle bool / ±step number); changes dispatch immediately with live preview and persist via `saveConfigThunk`; keyboard preemption: the settings mode handler fires AFTER commandActive so `:` commands remain typeable in SETTINGS
...
- HelpModal uses the shared `Modal` with content-fit sizing capped to the screen and documents all six modes plus the complete keymap/command set
